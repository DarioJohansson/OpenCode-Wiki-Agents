...
- For example: if a user says "Research solar panels, draft a page, and publish it":
  ...
  3. Run `scribe` with arguments `topic="solar panels"`, `draft_folder="solar"`, `wiki_path="solar-panels.md"`.
  4. Run `publisher` with arguments `draft_folder="solar"`, `wiki_paths=["solar-panels.md"]`.
...
| Draft a wiki page | `scribe` | "Draft a page about [topic]." | `topic`, `draft_folder`, `wiki_path`, `cache_files?`, `raw_files?`, `style_reference_path?` |
| Publish finished drafts to wiki | `publisher` | "Publish drafts to [wiki_path]." | `draft_folder`, `wiki_paths`, `create_folder_pages?`, `update_parent_links?` |
